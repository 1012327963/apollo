...
EdgeFollowMap::EdgeFollowMap() : hdmap_(hdmap::HDMapUtil::BaseMapPtr()) {}

bool EdgeFollowMap::CanProcess(const planning::PlanningCommand &command) const {
  return command.has_lane_follow_command();
}

hdmap::LaneWaypoint EdgeFollowMap::ToLaneWaypoint(
...
hdmap::LaneSegment EdgeFollowMap::ToLaneSegment(
...
void EdgeFollowMap::UpdateNextRoutingWaypointIndex(int cur_index) {
  if (cur_index < 0) {
...
std::vector<routing::LaneWaypoint> EdgeFollowMap::FutureRouteWaypoints() const {
...
void EdgeFollowMap::GetEndLaneWayPoint(
...
hdmap::LaneInfoConstPtr EdgeFollowMap::GetLaneById(const hdmap::Id &id) const {
  if (nullptr == hdmap_) {
    return nullptr;
  }
...
bool EdgeFollowMap::IsValid(const planning::PlanningCommand &command) const {
...
void EdgeFollowMap::UpdateRoutingRange(int adc_index) {
  // Track routing range.
...
bool EdgeFollowMap::UpdateVehicleState(const VehicleState &vehicle_state) {
...
bool EdgeFollowMap::UpdatePlanningCommand(
    const planning::PlanningCommand &command) {
  if (!CanProcess(command)) {
    AERROR << "Command cannot be processed by EdgeFollowMap!";
    return false;
  }
...
int EdgeFollowMap::SearchForwardWaypointIndex(
...
int EdgeFollowMap::SearchBackwardWaypointIndex(
    int start, const hdmap::LaneWaypoint &waypoint) const {
...
int EdgeFollowMap::NextWaypointIndex(int index) const {
...
int EdgeFollowMap::GetWaypointIndex(const hdmap::LaneWaypoint &waypoint) const {
...
bool EdgeFollowMap::PassageToSegments(routing::Passage passage,
...
std::vector<int> EdgeFollowMap::GetNeighborPassages(
...
bool EdgeFollowMap::GetRouteSegments(
  ...
  const double look_forward_distance =
      vehicle_state.linear_velocity() * FLAGS_look_forward_time_sec >
          ...
          : FLAGS_edge_follow_look_forward_short_distance;
  const double look_backward_distance = FLAGS_edge_follow_look_backward_distance;
...
bool EdgeFollowMap::GetNearestPointFromRouting(
...
hdmap::LaneInfoConstPtr EdgeFollowMap::GetRouteSuccessor(
...
hdmap::LaneInfoConstPtr EdgeFollowMap::GetRoutePredecessor(
    hdmap::LaneInfoConstPtr lane) const {
...
bool EdgeFollowMap::ExtendSegments(const hdmap::RouteSegments &segments,
...
bool EdgeFollowMap::ExtendSegments(
...
void EdgeFollowMap::AppendLaneToPoints(
...
void EdgeFollowMap::UpdateRouteSegmentsLaneIds(
...
apollo::hdmap::LaneWaypoint EdgeFollowMap::GetAdcWaypoint() const {
  return adc_waypoint_;
}

double EdgeFollowMap::GetDistanceToDestination() const {
