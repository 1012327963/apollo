...
void EdgeFollowStage::RecordObstacleDebugInfo(
...
StageResult EdgeFollowStage::Process(
...
StageResult EdgeFollowStage::PlanOnReferenceLine(
    const TrajectoryPoint& planning_start_point, Frame* frame,
    ReferenceLineInfo* reference_line_info) {
  if (!reference_line_info->IsChangeLanePath()) {
...
SLPoint EdgeFollowStage::GetStopSL(const ObjectStop& stop_decision,
